...
// Function to run the existing script
function runScript(firstRun, currentEl) {
    document.querySelectorAll("a[href*='minhluanpham1309.github.io']").forEach(el => {
        if (firstRun || currentEl === el.getAttribute('href')) {
            console.log("success");
            firstRun = false;
        }
    });
...
// Initial call to run the existing script
runScript(true);
        ...
        if (mutation.type === 'childList') {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === Node.ELEMENT_NODE) {
                    if (node.tagName === 'A') {
                        console.log("Tag A: " + node.tagName);
                        runScript(node.href);
                    } 
                }
            });
...
observer.observe(document.body, {
    ...
    attributeFilter: ['href', 'method'] // Specify which attributes to observe
});